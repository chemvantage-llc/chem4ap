...
public class LTIRegistration extends HttpServlet {
	...
	throws ServletException, IOException {
		...
		try {
			...
			if ("config".contentEquals(userRequest)) {
				...
				out.println(Util.head("LTI Registration") + clientIdForm(token) + Util.foot());
			} else {
				out.println(Util.head("LTI Registration") + registrationForm(request,null) + Util.foot());
			}
		} catch (Exception e) {
	...
	throws ServletException, IOException {
		...
		try {
			if ("finalize".contentEquals(userRequest)) {				
				...
				Algorithm algorithm = Algorithm.HMAC256(Util.getHMAC256Secret());
				JWT.require(algorithm).withIssuer(iss).build().verify(token);
				out.println(Util.head("Chem4AP LTI Registration") + "<h1>Chem4AP Registration</h1>" + createDeployment(request) + Util.foot());			
			} else {
				...
				if (dynamicRegistration) {
					...
					out.println(Util.head("Chem4AP LTI Registration") + "<h1>Chem4AP</h1>" + "<h2>Registration Success</h2>Thank you. A registration email has been sent to your address.<p>" + Util.foot());			
				}
			}
		} catch (Exception e) {
			...
			if (dynamicRegistration) {
				String emailmessage = message + "<br/>"
						...
						+ debug.toString();
				Util.sendEmail("Chem4AP Administrator","admin@chemvantage.org","Dynamic Registration Error",emailmessage);
			}
			out.println(Util.head("LTI Registration") + registrationForm(request,message) + Util.foot());
		}
	}
	...
	String registrationForm(HttpServletRequest request, String message) {
		...
		String AcceptChem4APTOS = request.getParameter("AcceptChem4APTOS");
		...
		buf.append("<form id=regform method=post action=/lti/registration>"
				+ "Please complete the form below to create a trusted LTI Advantage connection between your LMS and Chem4AP "
				+ "that is convenient, secure and <a href=https://site.imsglobal.org/certifications/chemvantage/chem4ap>certified by 1EdTech</a>. "
				+ "When you submit the form, Chem4AP will send "
		...
		buf.append("<label><input type=checkbox required name=AcceptChem4APTOS value=true " + ((AcceptChem4APTOS!=null && AcceptChem4APTOS.equals("true"))?"checked":"") + " />Accept the <a href=/terms_and_conditions.html target=_blank aria-label='opens new tab'>Chem4AP Terms of Service</a></label><br/><br/>\n");

		if (!dynamic) {  // show recaptcha tool
			buf.append("<div class='g-recaptcha' data-sitekey='" + Util.getReCaptchaSiteKey() + "' aria-label='Google Recaptcha'></div><br/><br/>"
	...
	void sendRegistrationEmail(String token, HttpServletRequest request) throws Exception {
		...
		buf.append("Thank you for your Chem4AP registration request.<p>");
		
		if (iss.contains("dev-chem4ap.appspot.com")) {
			buf.append("Chem4AP is pleased to provide access to our software development server for testing LTI connections. "
			...
			buf.append("<h3>Getting Started</h3>"
					+ "When you complete the registration steps below, your account will be activated immediately. You may create Chem4AP "
					...
					+ "video lectures, and in-class polls. In order to access the assignments, students must subscribe to the Chem4AP service "
					+ "for $" + price + ".00 USD per month (1 month minimum). As a reminder, access to Chem4AP by instructors and LMS account "
		...
		buf.append("The next step is to enter the Chem4AP configuration details into your LMS. "
				+ "This will enable your LMS to communicate securely with Chem4AP. Normally, "
				...
				+ "Chem4AP is an Open Education Resource for AP Chemistry. Learn more about Chem4AP "
				+ "<a href=https://www.chem4ap.com>here</a>.<p>"
				+ "Follow the detailed instructions for installing Chem4AP in your LMS at "
				+ "<a href=https://www.chem4ap.com/install.html>https://www.chem4ap.com/install.html</a>.");
		
		buf.append("<h3>After you finish installing Chem4AP in your LMS</h3>"
				...
				+ "configuration URLs to Chem4AP. If the link expires, click <a href=" + iss + "/lti/registration>here</a> to get anther one.<p>");
		...
		Util.sendEmail(name,email,"Chem4AP LTI Registration",buf.toString());
	}
	...
	String clientIdForm(String token) {
		...
		try {
			...
			buf.append("<h2>To the LMS Administrator:</h2>"
					+ "By now you should have configured your LMS to connect with Chem4AP, and you should have "
			...
			switch (lms) {
			case "blackboard":
				client_id = (iss.equals("https://dev-chem4ap.appspot.com")?"ec076e8c-b90f-4ecf-9b5d-a9eff03976be":"be1004de-6f8e-45b9-aae4-2c1370c24e1e");
				...
				break;
			case "schoology":
				client_id = (iss.equals("https://www.chem4ap.com")?"6558245496":"");
				...
				buf.append("The Schoology admin can get the Deployment ID value for Chem4AP by clicking the "
						+ "Apps icon > App Center > " + (iss.equals("https://www.chem4ap.com")?"Organization Apps. ":"My Developer Apps. ") 
						+ "Find Chem4AP and select Configure. The Deployment ID should be two large (~10-digit) numbers separated by a hyphen. "
						+ (iss.equals("https://dev-chem4ap.appspot.com")?"The Client ID value is the first 10-digit number in the Deployment ID.":"") 
						+ "<p>");
				break;
			case "canvas":
				...
				buf.append("Canvas uses the developer key as the Client ID, so enter that value from the list of "
						...
						+ "settings menu for Chem4AP. It is a compound value that consists of a number and a hex string "
	...
	String getConfigurationJson(String iss,String lms) {
		...
		config.addProperty("title","Chem4AP" + (iss.contains("dev")?" Development":""));
		config.addProperty("description", "Chem4AP is an Open Education Resource for teaching and learning AP Chemistry");;
		config.addProperty("privacy_level", "public");
		config.addProperty("target_link_uri", iss + "/lti");
		...
		config.add("scopes", scopes);
		  JsonArray extensions = new JsonArray();
		    ...
		    ext.addProperty("platform", "canvas.instructure.com");
		      ...
		      settings.addProperty("text", "Chem4AP" + (iss.contains("dev")?" Development":""));
		      settings.addProperty("icon_url", iss + "/images/chem4ap_atom.png.png");
		        JsonArray placements = new JsonArray();
		         JsonObject plcmnt1 = new JsonObject();
		          plcmnt1.addProperty("text", "Chem4AP" + (iss.contains("dev")?" Development":""));
		          plcmnt1.addProperty("enabled", true);
		          plcmnt1.addProperty("icon_url", iss + "/images/chem4ap_atom.png.png");
		          plcmnt1.addProperty("placement", "assignment_selection");
		          plcmnt1.addProperty("message_type", "LtiDeepLinkingRequest");
		          plcmnt1.addProperty("target_link_uri", iss + "/lti");
		        placements.add(plcmnt1);
		         JsonObject plcmnt2 = new JsonObject();
		          plcmnt2.addProperty("text", "Chem4AP" + (iss.contains("dev")?" Development":""));
		          plcmnt2.addProperty("enabled", true);
		          plcmnt2.addProperty("icon_url", iss + "/images/chem4ap_atom.png.png");
		          plcmnt2.addProperty("placement", "editor_button");
		          plcmnt2.addProperty("message_type", "LtiDeepLinkingRequest");
		          plcmnt2.addProperty("target_link_uri", iss + "/lti");
		        placements.add(plcmnt2);
		         JsonObject plcmnt3 = new JsonObject();
		          plcmnt3.addProperty("text", "Chem4AP" + (iss.contains("dev")?" Development":""));
		          plcmnt3.addProperty("enabled", true);
		          plcmnt3.addProperty("icon_url", iss + "/images/chem4ap_atom.png.png");
		          plcmnt3.addProperty("placement", "link_selection");
		          plcmnt3.addProperty("message_type", "LtiDeepLinkingRequest");
		          plcmnt3.addProperty("target_link_uri", iss + "/lti");
	...
	JsonObject postRegistrationRequest(JsonObject openIdConfiguration,HttpServletRequest request) throws Exception {
		...
		regJson.addProperty("client_name", "Chem4AP" + (iss.contains("dev-chem4ap")?" Development":""));
		regJson.addProperty("jwks_uri", iss + "/jwks");
		regJson.addProperty("logo_uri", iss + "/images/chem4ap_atom.png.png");
		...
		ltiToolConfig.addProperty("target_link_uri", iss + "/lti");
		...
		deepLinking.addProperty("target_link_uri", iss + "/lti");
		...
		resourceLaunch.addProperty("target_link_uri", iss + "/lti");
	...
	Deployment createNewDeployment(JsonObject openIdConfiguration, JsonObject registrationResponse, HttpServletRequest request) throws Exception {
		try {
			...
			try {
				...
				Util.sendEmail("Chem4AP Administrator","admin@chemvantage.org","Dynamic Registration Error: LMS Type Unknown",openIdConfiguration.toString());
			}
			...
			throw new Exception("Failed to create new deployment in Chem4AP: " + e.toString() + "<br/>OpenId Configuration: " + openIdConfiguration.toString() + "<br/>Registration Response: " + registrationResponse.toString());
		}
	}
	...
	String successfulRegistrationRequestPage(JsonObject openid_configuration, HttpServletRequest request) {
		...
		if (request.getServerName().contains("dev-chem4ap.appspot.com")) {
			buf.append("Chem4AP is pleased to provide access to our software development server for testing LTI connections. "
			...
			buf.append("Your Chem4AP has been fully activated and provisioned with 1 free student license for testing. Each unique student "
					+ "login will use one license. You may purchase additional licenses in bulk directly from Chem4AP at a discount. "
					+ "Otherwise, Chem4AP will charge each student a subsciption price of $" + price + ".00 USD per month to access the assignments. "
					+ "As a reminder, access to Chem4AP by instructors and LMS account administrators is always free.<br/><br/>");
		}
		...
		case "moodle":
			buf.append("<h2>For the Instructor</h2>"
					+ "To add Chem4AP assignments to your course, turn editing ON and:<ol>"
					+ "<li>Click 'Add an activity or resource'</li>"
					+ "<li>Click 'External Tool'</li>"
					+ "<li>Select Chem4AP from preconfigured tools and click 'Select content'</li>"
					+ "<li>Choose one or more Chem4AP assignments, click 'Submit' and then 'Continue'</li>"
					+ "</ol>");
			break;
		case "canvas":
			buf.append("<h2>For the Instructor</h2>"
					+ "To add Chem4AP assignments to your course, go to the Assignments page:<ol>"
					+ "<li>Click the red '+ Assignment' button</li>"
					+ "<li>For Submission Type select 'External Tool'</li>"
					+ "<li>Click Find and select Chem4AP from preconfigured tools</li>"
					+ "<li>Choose a Chem4AP assignment and topic, then click 'Create this assignment' and then 'Select'</li>"
					+ "<li>Click 'Save' or 'Save and Publish'</li>"
					+ "</ol>");
			break;
		default:
			buf.append("<h2>For the Course Instructor:</h2>"
					+ "Although we do not have specific instructions for how to add a Chem4AP assignment to your course in " + lms + ", "
					+ "in general you should navigate to your course page and<ol>"
					+ "<li>Add a new assignment, content or resource</li>"
					+ "<li>Select Chem4AP from a list of preconfigured tools</li>"
					+ "<li>Select one or more Chem4AP assignments to add</li>"
	...
	static void sendApprovalEmail(Deployment d, HttpServletRequest request) {
		...
		if (iss.contains("dev-chem4ap.appspot.com")) {
			buf.append("Chem4AP is pleased to provide free access to our software development server for testing LTI connections. "
					+ "Please note that the server is sometimes in an unstable state, and accounts may be reset or even deleted at any time. ");
		} else {
			buf.append("<h3>Getting Started</h3>"
					+ "Your Chem4AP account is now active, and you may create new placement exams and assignments, or just expolore the "
					+ "site without limitations. "
					+ "Students must purchase a Chem4AP subscription for $" + price + ".00 USD per month to access the assignments. "
					+ "As a reminder, access to Chem4AP by instructors and LMS account administrators is always free.");
		}
		
		buf.append("<h3>Helpful Hints</h3>"
				+ "Chem4AP supports two types of LTI launches from your LMS:<ol>"
				+ "<li>Deep Linking - used by the instructor, course designer or administrator to select Chem4AP assignments.</li>"
				+ "<li>Resource Link - used by students to launch an existing assignment.</li>"
				+ "</ol>"
				+ "You should configure the Chem4AP placements in your LMS with the appropriate locations and permissions.<br/><br/>");
		...
		case "canvas":
			buf.append("To the Course Instructor:<ol>"
					...
					+ " <li>External Tool URL: Find Chem4AP or enter " + iss + "/lti</li>"
		...
		case "blackboard":
			...
			buf.append("<ol><li>Go to the course | Content | Build Content | Chem4AP</li>"
					...
					+ "</ul></li>"
					+ "<li>Submit</li>"
					+ "<li>Click the new assignment link to launch Chem4AP</li>"
		...
		default:
			buf.append("To the Course Instructor:<br>"
					+ "Although we do not have specific instructions for how to add a Chem4AP assignment to your course in " + d.lms_type + ", "
					+ "in general you should navigate to your course page and<ol>"
					+ "<li>Add a new assignment, content or resource</li>"
					+ "<li>Select Chem4AP from a list of preconfigured tools</li>"
					+ "<li>Select one or more Chem4AP assignments to add</li>"
		...
		try {
			Util.sendEmail(d.contact_name,d.email,"Chem4AP Registration",buf.toString());
		} catch (Exception e) {
		}
	}
}
